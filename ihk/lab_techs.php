<?php
	...
	echo("<center>");
	echo("<h1><u>"."Laboratory Technicians"."</u></h1>");
	echo("<hr>");
	echo("<form method='POST' action='add_labtech.php'>
			<br>
	...
	if($result->num_rows>0){
		...
		while($row = $result->fetch_array()){
			...
			echo("<td>$row[1]</td>
				...
				<td>$row[4]</td>
				<td>
					<form method='POST' action='delete_labtech.php'>
						<input type='hidden' value='$pk' name='primary'>
						<input id='delete' type='submit' value='Delete'>
					</form>
				<td>
				<form method='POST' action='update_labtech.php'>
					<input type='hidden' value='$pk' name='primary'>
